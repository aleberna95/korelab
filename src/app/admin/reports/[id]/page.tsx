    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 space-y-8">
      ...
      <section className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          ...
          <p className="text-2xl font-bold text-green-700">{report.metrics.uptimePct.toFixed(3)}%</p>
        </div>
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          <p className="text-xs text-gray-500 mb-1">Downtime</p>
          <p className="text-2xl font-bold text-gray-900">{fmtDuration(report.metrics.downtimeSec)}</p>
        </div>
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          <p className="text-xs text-gray-500 mb-1">Incidents</p>
          <p className="text-2xl font-bold text-gray-900">{report.metrics.incidentCount}</p>
        </div>
        {report.metrics.mttrSec !== undefined && (
          <div className="bg-white border border-gray-200 rounded-xl p-4">
            <p className="text-xs text-gray-500 mb-1">MTTR</p>
            <p className="text-2xl font-bold text-gray-900">{fmtDuration(report.metrics.mttrSec)}</p>
          </div>
        )}
        {report.metrics.avgResponseMs !== undefined && (
          <div className="bg-white border border-gray-200 rounded-xl p-4">
            <p className="text-xs text-gray-500 mb-1">Avg response</p>
            <p className="text-2xl font-bold text-gray-900">{report.metrics.avgResponseMs}ms</p>
          </div>
        )}
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          <p className="text-xs text-gray-500 mb-1">Checks</p>
          <p className="text-2xl font-bold text-gray-900">{report.metrics.checks.toLocaleString()}</p>
        </div>
      </section>
      ...
      {report.incidents.length > 0 && (
        <section className="space-y-3">
          <h2 className="text-sm font-semibold text-gray-700 uppercase tracking-wide">Incidents</h2>
          <div className="space-y-2">
            {report.incidents.map((inc) => (
              <div key={inc.id} className="bg-white border border-gray-200 rounded-lg px-4 py-3">
                <div className="flex justify-between items-start gap-3">
                  <p className="font-medium text-sm text-gray-900">{inc.title}</p>
                  <span className="text-xs text-gray-500 shrink-0 capitalize">{inc.severity}</span>
                </div>
                {inc.publicMessage && (
                  <p className="text-xs text-gray-500 mt-1">{inc.publicMessage}</p>
                )}
      ...
      {report.maintenance.length > 0 && (
        <section className="space-y-3">
          <h2 className="text-sm font-semibold text-gray-700 uppercase tracking-wide">Maintenance</h2>
          <div className="space-y-2">
            {report.maintenance.map((mw) => (
              <div key={mw.id} className="bg-white border border-gray-200 rounded-lg px-4 py-3">
                <p className="font-medium text-sm text-gray-900">{mw.title}</p>
                <p className="text-xs text-gray-400 mt-1">
        ...
        <section className="space-y-3">
          ...
          {report.notes.client && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg px-4 py-3">
              <p className="text-xs text-blue-600 mb-1">Client notes (visible to client)</p>
              <p className="text-sm text-gray-900">{report.notes.client}</p>
            </div>
          )}
          {report.notes.private && (
            <div className="bg-gray-50 border border-gray-200 rounded-lg px-4 py-3">
              <p className="text-xs text-gray-500 mb-1">Private notes (admin only)</p>
              <p className="text-sm text-gray-700">{report.notes.private}</p>
            </div>
          )}
        </section>
      )}
